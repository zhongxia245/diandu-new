...
import { Drag, getImageWH } from '@create/utils/_index'
import { savePageTpl, getPageTpl, uploadBase64Img } from '@create/ajax/_ajax.js'
import { Point } from '@create/components/_index'
import { Tools } from '../_index'
import { ALLOW_ADD_POINT, initEvent } from './_event_handle'
import {
  POINT_SIZE,
  PAGE_SIZE,
  PRE_PAGE_ID,
        ...
        onPress: val =>
          new Promise((resolve, reject) => {
            if (!val) {
              ...
              html2canvas(
                document.querySelector(`#${PRE_PAGE_ID}${this.props.pageIndex}`)
              ).then(canvas => {
  ...
  renderPage() {
    ...
    let JsxDom = (
      <div className="pageitem-content__tip">
        <h3>提示</h3>
        <p>
          手机横屏比例为16:9，对应像素通常为1920*1080。建议上传小于此分辨率的图片作为背景图
        </p>
        <p>
    ...
    return (
      <div className={`pageitem ${PRE_PAGE_CLASS + pageIndex}`}>
          ...
          <div className="pageitem__header">
            <div className="pageitem-header__upload">
              ...
              <div>
                ...
                <div
                  className="u-btn u-btn--small u-btn--red"
                  onClick={this.handleDelPage}
                >
                  删除
                </div>
              </div>
        ...
        <div className="pageitem__right">
          {/* 工具栏 */}
          <Tools
            pointIndex={currentSelectPointIndex}
            pageIndex={pageIndex}
            form={form}
          />
        </div>
